...
class Trainer(DefaultTrainer):
    ...
    def __init__(self, cfg):
        ...
        model = self.build_model_with_gpu_stats(cfg)
    ...
    def build_model_with_gpu_stats(self, cfg):
        """Just for debugging GPU memory usage"""
        torch.cuda.empty_cache()
        torch.cuda.reset_peak_memory_stats()
        mem_before = torch.cuda.memory_allocated()
        model = self.build_model(cfg)
        mem_after = torch.cuda.memory_allocated()
        print(f"Memory allocated for model: {(mem_after - mem_before) / 1024 / 1024} MB")
        return model
...
def register_3lc_hooks(trainer, cfg):
    ...
    trainer.register_hooks([metrics_collection_hook, d2_metrics_collection_hook])
### 3LC END ###
